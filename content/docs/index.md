...
The most inportant colors are defined in the `src/layouts/Default.vue` file at the top of the `style` block via CSS variables. If you want to change the brand color to orange for example, you would simply touch that value there.

```css
:rrot {
	--color-brand: theme('colors.orange.600');
}
```
